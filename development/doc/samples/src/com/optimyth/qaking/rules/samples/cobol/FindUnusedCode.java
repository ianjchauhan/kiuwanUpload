...
public class FindUnusedCode extends AbstractCobolRule {
  ...
  @Override protected void visit(BaseNode root, RuleContext ctx) {
    ...
    Procedures units = new Procedures(pd);
    UsedChecker checker = units.buildUsedChecker();
    ...
    Set<CobolStatement> statements = findStatements(pd);
  ...
  private Set<CobolStatement> findStatements(ProcedureDivision pd) {
    Set<CobolStatement> statements = Sets.newHashSetWithExpectedSize(256);
    ...
    pd.accept(new NodeVisitor() {
      public void visit(BaseNode node) {
        if(node instanceof CobolStatement) {
          ...
          if( !NOP_STATEMENT_PRED.test(stmt) && !inDeclaratives(stmt) ) {
            statements.add(stmt);
          }
        }
      }
  ...
  private void removeReachableStatements(DataFlowGraph<DataFlowNode> cfg, UsedChecker checker, Set<CobolStatement> statements) {
  // Visit all statements reachable in control-flow
    ...
    ControlFlowNavigator.forwardBFT(cfg, cfgNode -> {
  ...
  private void reportUnused(UsedChecker checker, Set<CobolStatement> unusedStmts, RuleContext ctx) {
    for(HLABehaviouralUnit unused : checker.unusedProcedures()) {
      if(CobolPredicates.TRIVIAL_PROCEDURE.test(unused)) continue; // Single EXIT or CONTINUE paragraph, ignore
      String msg = "unused " + (unused instanceof ProcedureSection ? "section " : "procedure ") + unused.getName();
    ...
    for(HLAStatement unusedStmt : unusedStmts) {
      ...
      if( where != null && !checker.isUnusedProcedure(where) ) {
        String msg = "unused " + unusedStmt.getTypeName();
