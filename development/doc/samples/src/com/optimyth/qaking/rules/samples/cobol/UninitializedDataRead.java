...
public class UninitializedDataRead extends AbstractCobolRule {
   ...
   */
  private void checkIllegalUsages(ProcedureDivision pd, DataFlowGraph<DataFlowNode> cfg, RuleContext ctx) {
    Set<DataEntry> initialized = Sets.newHashSetWithExpectedSize(128);
    
    // A common case is a table of constants (like messages), control such case
    markInitializedTables(pd, initialized);
        
    ControlFlowNavigator.forwardDFT(cfg, node -> {
  ...
  private void markInitializedTables(ProcedureDivision pd, Set<DataEntry> initialized) {
  ...
  private void checkDataReference(BaseNode stmt, DataReference ref, Set<DataEntry> initialized, RuleContext ctx) {
  ...
  private void markAsInitialized(DataEntry dataEntry, Set<DataEntry> initialized) {
    dataEntry.accept(subfield -> {
  ...
  private boolean checkInitialized(DataEntry de, Set<DataEntry> initialized) {
    ...
    if(de.isRedefines()) {
      ...
      return target != null && initialized.contains(target);
    }
    return false;
  }
}
