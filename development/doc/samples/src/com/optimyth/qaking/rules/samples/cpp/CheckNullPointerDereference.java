...
import com.kiuwan.qaking.cpp.ast.SwitchStatement;
...
import com.optimyth.qaking.codeanalysis.metadata.model.functional.FunctionDescriptor;
import com.optimyth.qaking.codeanalysis.metadata.model.functional.FunctionSignature;
import com.optimyth.qaking.cpp.ast.CppNode;
...
import static com.optimyth.qaking.cpp.ast.CppNode.cast;
...
public class CheckNullPointerDereference extends AbstractCppRule {
  ...
  private boolean isMemoryAccessedWithoutNullCheck(CppNode call, NullPointerFunction npf, RuleContext ctx) {
    ...
    NullCheckProblem problem = NullCheckProblem.compile(call, npf);
    if(problem == null) return false; // nothing could be said

    // Prepare flow graph
    DataFlowNode start = getDataFlowNode(call, ctx);
    if(start==null) return false;

    AtomicBoolean isNullChecked = new AtomicBoolean(false);
    AtomicBoolean isMemoryAccessedWithoutNullCheck = new AtomicBoolean(false);
    AtomicBoolean isPotentialNullReturned = new AtomicBoolean(false);
   ...
   */
  private void analyzeNullCheck(NullCheckProblem problem, DataFlowNode start, AtomicBoolean nullChecked, AtomicBoolean memoryAccessedWithoutNullCheck, AtomicBoolean potentialNullReturned) {
    ControlFlowNavigator.forwardBFT(start, new ControlFlowVisitor() {
      public boolean onDataFlowNode(IDataFlowNode node) {
        ...
        for (IVarRef va : node.getVariableAccesses()) {
          if (problem.getVarDeclarator().equals(va.getDeclaration())) {
            ...
            CppNode unaryExp = cast(va.getReference());
            ...
            if (isErrnoChecked(cast(va.getReference()))) {
              nullChecked.set(true);
      ...
      private boolean isErrnoChecked(CppNode unaryExp) {
        return unaryExp.hasAncestor(BOOLEAN_EXPR) || unaryExp.ancestor("expression").parent() instanceof SwitchStatement;
      }
      ...
      private boolean isPtrDereference(CppNode unaryExp) {
  ...
  private boolean isNullChecked(CppNode unaryExp) {
  ...
  private static final class NullPointerFunction {
    ...
    private static NullPointerFunction create(CppNode fdef, int argPos) {
      FunctionSignature signature = FunctionUtil.functionSignature(fdef, TypedefResolver.NULL);
      FunctionDescriptor fd = new FunctionDescriptor(signature, signature.toString(), null);
      fd.setStandard("__custom__");
      fd.getTags().add("nullptr:-1");
      return new NullPointerFunction(fd, argPos);
    }

    public String getFunctionName() { return functionDescriptor.getName(); }
    public boolean isErrnoSupported() { return "errno".equals(functionDescriptor.getErrorProcessing()); }
  ...
  private static final class NullCheckProblem {
    ...
    private final CppNode varDeclarator; // declaration of target var (candidate null pointer) to check

    private NullCheckProblem(NullPointerFunction nullPointerFunction, CppNode varDeclarator) {
    ...
    public CppNode getVarDeclarator() { return varDeclarator; }

    private static NullCheckProblem compile(CppNode call, NullPointerFunction npf) {
      CppNode exp = FunctionUtil.getExpression(call, npf.getArgPos());
      if(exp.isNull()) return null; // nothing could be said

      CppNode varDeclarator = CppNode.NULL;
      ...
      if(exp.isTypeName("declarator")) {
        ...
        CppNode ue = exp.find("unary_expression");
        if(ExpressionUtil.isSimpleVarReference(ue)) {
          varDeclarator = DeclarationUtil.getVariableDeclarator(ue).topmostAncestor("declarator");
        }
      }

      // No LHS simple varname nor declaration found
      if(CppNode.isNull(varDeclarator)) return null;
