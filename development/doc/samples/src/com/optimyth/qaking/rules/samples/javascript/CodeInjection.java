...
import com.optimyth.qaking.codeanalysis.metadata.model.oo.ClassDescriptor;
import com.optimyth.qaking.js.ast.JSFunctionCall;
...
import com.optimyth.qaking.js.tainting.typesystem.TypeContext;
...
import static com.optimyth.qaking.js.utils.FunctionUtil.fetchtFunctionDescriptor;
import static com.optimyth.qaking.js.utils.FunctionUtil.findReceiverType;
...
public class CodeInjection extends AbstractJavaScriptRule {
  ...
  private static final String DEFAULT_CHECKERS = "eval,execScript,Function,setInterval,setTimeout,executeScript";
  ...
  private static interface InsanityChecker {
    public boolean match(JSNode call, RuleContext ruleCtx, TypeContext ctx);
  }
  ...
  // setTimeout()
  private static final InsanityChecker EXECUTE_SCRIPT = (call, ruleCtx, ctx) -> {
    if (!(call.firstChild() instanceof JSPropertyGet)) return false;

    ClassDescriptor container = findReceiverType((JSPropertyGet) call.firstChild(), ctx);
    FunctionDescriptor fdesc = fetchtFunctionDescriptor((JSFunctionCall) call, ((JSFunctionCall) call).getFunctionName(), container, ctx);

    return fdesc != null && "executeScript".equals(fdesc.getName()) && "InAppBrowser".equals(fdesc.getContainer().getName());
  };
  ...
  private static final Map<String, InsanityChecker> CHECKERS = ImmutableMap.<String, InsanityChecker>builder()
    ...
    .put("executeScript", EXECUTE_SCRIPT)
    .build();
}
