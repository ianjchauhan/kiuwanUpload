...
public class ProperCustomException extends AbstractPhpRule {
  ...
  private boolean checkConstructor(PhpNode clazz) {
    ...
    String superClazz = ClassUtil.getSuperclass(clazz);
    ...
    return cons.child(STATEMENT_BLOCK).has((NodePredicate) node -> {
  ...
  private boolean checkToString(PhpNode clazz) {
    return clazz.has((NodePredicate) function ->
      FUNCTION_DECL.is(function) && "__toString".equals(getFunctionName(function))
    );
  }
