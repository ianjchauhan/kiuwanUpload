...
public class AvoidConcatJdbcStatement extends AbstractRule {
  ...
  private final Query jdbcCalls = query().find( variables(JDBC_STATEMENT::test) ).find(JDBC_CALL, navigateToCalls);
  
  @Override protected void visit(BaseNode root, RuleContext ctx) {
    NodeSet calls = jdbcCalls.run(this, ctx, (BaseNode) ctx.getHighLevelTree()).current();
